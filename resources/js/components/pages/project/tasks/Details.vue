...
const getTask = () => {
  ...
  axios.get(`/api/v1/tasks/${route.query.task}`, {
    params: {
...
onMounted(() => {
  ...
  Echo
      .private(`task-updated.task.${route.query.task}`)
...
const addToClipboard = async (value) => {
  try {
    await toClipboard(window.location.href)
      ...
      setTimeout(() => tippyMessage.value = 'Click to copy link for task', 2000);
  } catch (e) {
  }
}

const updateTask = (params) => {
  axios.put(`/api/v1/tasks/${route.query.task}`, {...params, sprint_id: route.params.sprint})
}
</script>
...
<template>
  <fwb-modal size="7xl" class="z-50" @close="closeTask">
    <template #header>
      <fwb-button color="light" class="mr-2" @click="createTasks.toggleCreateTaskModal">
    ...
    <template #body>
      <div class="grid grid-cols-1 z-50 md:grid-cols-3 gap-4" :key="route.query.task + JSON.stringify(data)">
        <div class="col-span-2">
          <div class="mb-6">
            <Label>Description</Label>
            <Editor v-if="data.description" class="input big-input" :model="data.description"
                    :editable="false"/>
            <div v-if="!data.description" class="input big-input">
          ...
          <fwb-tabs v-model="activeTab" variant="pills" class="p-5 ">
            ...
            <fwb-tab name="comments" title="Comments" :disabled="!data.chat_id">
              <Comments v-if="activeTab === 'comments'" :chat-id="data.chat_id"/>
            </fwb-tab>
        ...
        <div class="flex flex-col gap-4">
            ...
            <Label class="flex gap-1 items-center">
                Code: {{ data.task_id }}
                <tippy @click="addToClipboard(data.task_id)"  :content="tippyMessage" :hide-on-click="false">
                    <CopyIcon class="w-4 h-4"/>
                </tippy>
            </Label>
          <Label>
            Due date: <span
              v-if="data.due_date">{{ DateTime.fromISO(data.due_date).toLocaleString(DateTime.DATE_SHORT) }}</span>
          </Label>
          <Label>
            Tags
            <div class="flex gap-1 mt-2 flex-wrap">
              <fwb-badge v-for="(str, index) in data.tags">{{ str }}</fwb-badge>
            </div>
          </Label>
          <Label>
            <div>
          ...
          <Label>Created by
            <User v-if="data.createdBy" show-name="true" :user="data.createdBy"/>
          </Label>
