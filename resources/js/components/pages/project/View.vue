...
const loadData = () => {
  axios
    .get(`/api/v1/projects/${route.params.project}`)
    .then(res => {
      const list = [
        {
          link: '/',
...
<template>
  <Auth>
    <ul
      class="hidden lg:flex flex-wrap font-medium text-center text-gray-500 dark:text-gray-400 text-sm"
    >
      <li>
        <router-link
          :to="{
    ...
    <router-view :key="route.params.project"></router-view>
