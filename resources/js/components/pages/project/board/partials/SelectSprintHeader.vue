...
const createSprint = () => {
    axios
        .post("/api/v1/sprints", {
            ...form,
            board_id: boardsState.activeBoard.id,
        })
        .then((res) => {
...
<template>
    <div>
        <fwb-dropdown text="Bottom">
            <template #trigger>
                <h1 class="flex gap-1 cursor-pointer items-center">
                    <div class="flex flex-col" :key="boardsState.activeSprint">
                        ...
                        <small
                            class="text-xs"
                            v-if="boardsState.activeSprint.start_time"
                        >
                            {{
                                DateTime.fromSQL(
                                    boardsState.activeSprint.start_time
                                ).toLocaleString(DateTime.DATE_SHORT)
                            }}
                            -
                            {{
                                DateTime.fromSQL(
                                    boardsState.activeSprint.end_time
                                ).toLocaleString(DateTime.DATE_SHORT)
                            }}
                        </small>
                    </div>
            ...
            <fwb-list-group>
                <fwb-list-group-item
                    v-for="obj in boardsState.getActiveBoard.sprint"
                    :key="obj"
                    class="cursor-pointer justify-between"
                >
                    {{ obj.title }}
                    <div
                        v-if="!obj.is_active && !obj.end_time"
                        class="hover:underline"
                        @click="() => setActiveSprint(obj)"
                    >
                        Activate
                    </div>
                    ...
                    <router-link
                        class="hover:underline"
                        v-if="obj.id !== boardsState.activeSprint.id"
                        :to="{ params: { sprint: obj.id } }"
                    >
                        Open
                    </router-link>
                </fwb-list-group-item>
                <fwb-list-group-item
                    v-if="can(`can-create-board.${route.params.project}`)"
                >
                    <div
        ...
        <Modal ref="createSprintRef">
            <template #header> Create new sprint </template>
            <template #body>
