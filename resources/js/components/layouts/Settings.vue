  ...
  <Auth>
    ...
    <div class="flex flex-col md:flex-row gap-4">
      <div class="md:sticky sticky-top">
        ...
        <fwb-list-group
          v-show="showMenu"
          class="min-w-full md:min-w-[300px] sticky top-1"
        >
          <fwb-list-group-item>
          ...
          >
            <div
              class="flex justify-between items-center w-full show-plus-on-hover"
            >
              <router-link :to="{ name: 'settings.roles' }">Roles</router-link>
              <fwb-button
                v-if="route.name === 'settings.roles'"
            ...
            <ul class="pl-3" v-if="route.name === 'settings.roles'">
              <li
                ...
                @click="() => roleStore.setActiveRole(obj.id)"
                :class="`cursor-pointer hover:underline ${
                  roleStore.activeRole === obj.id ? 'font-bold' : ''
                }`"
              >
                {{ obj.title }}
              </li>
