...
const save = () => {
    axios
        .put(`/api/v1/roles/${roleStore.role.id}`, {
            permissions: Object.keys(selected.value).filter(
        ...
        .then(() => {
            ...
            toast.success(
                `Permissions updated for ${roleStore.role.title} role`,
...
<template>
    <Modal ref="modalRef">
        <template #header> Select permissions for role </template>
        <template #button>
