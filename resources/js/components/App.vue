<template>
    ...
    <InteractiveToast ref="toastRef">
        ...
        <template #actions>
            ...
            }" target="_blank" :class="{'flex  w-full': true}">

                <fwb-badge type="default" class="flex justify-start w-full gap-2 py-2 px-4">
...
watch(() => userState.user.id, () => {
    if (!userState.user.id) {
        return;
    }
    loadNumberOfUnreadMessages();

    window.Echo.private(`start-call.${userState.user.id}`)
        .listen('StartVideoCall', (data) => {
            console.log(data)
            call.value = data.callId;
            toastRef.value.showToastFn();
            if (!document.hasFocus()) {
                const notification = new Notification("Video call", {body: `${data.callId.user.name} is calling you.`});
    ...
    window.Echo.private('UpdateChatForUser.' + userState.user.id)
        .listen('ChatUpdate', (data) => {
            loadNumberOfUnreadMessages()
            if (!document.hasFocus()) {
               const notification = new Notification("New message", {body: `${data.message.user} send you new message`});
