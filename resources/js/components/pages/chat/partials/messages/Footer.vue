    ...
    <form @submit.prevent="submit" class="border-t border-amber-50 dark:border-gray-900 p-6">
        ...
        <div class="relative">
            <div v-if="activeMessage" class="p-1 text-sm  font-bold ">
                ...
                <Editor class="h-4 overflow-hidden" :editable="false"
                        :model="activeMessage.message"
                />
            </div>
            <Editor ref="editorRef" v-model="chatMessageStore.message" css-class="input"
                    @submitted="submit"
            />
...
const activeMessage = computed(() => {
    return _.find(chatMessageStore.messages, {id: chatMessageStore.reply_to_message_id})
})
const submit = () => {
