...
watch(() => deleteRoleToastRef.value?.showToast, () => {
    roleName.value = '';
}, {
    deep: true,
    immediate: true
        ...
        <div class="w-full" :key="roleStore.role.title">
            <div class="flex justify-between items-center">
                ...
                <interactive-toast ref="deleteRoleToastRef">
                    ...
                    <template #content>
                        You are about to delete role <b>{{ roleStore.role.title }}</b>!
                        All permissions that users have in this role will be lost!!!
                        <br>

                        <fwb-input v-model="roleName" label="To delete role type in its name:"
                                   :placeholder="roleStore.role.title" size="sm"/>
                    </template>
                    <template #actions>
                        <fwb-button :disabled="shouldDeleteButtonBeDisabled" size="xs" color="red" @click="deleteRole">
                            Yes delete it!
                        </fwb-button>
                    </template>
                </interactive-toast>
