...
const results = computed(() => {
  ...
  const withoutMe = response.value.data.filter(obj => obj.id !== userState.user.id);
    ...
    <template #body>
      ...
      <div :class="{
        ...
        <Label
            ...
            :forInput="'title'" :disabled="selected.length < 2">Group chat title</Label>
        <Text id="title"
              :class="{
                    'cursor-not-allowed !placeholder-gray-200':selected.length < 2
                    }"
              :disabled="selected.length < 2"
              v-model="title" placeholder="Group chat title"/>
        <Errors name="title"/>
      </div>
      <hr>
      <div class="mt-4">
        <Label :forInput="'searchUsers'">Search users</Label>
        <Text id="searchUsers" v-model="searchUsers" placeholder="Search users"/>
      </div>

      <ul class="divide-y divide-gray-200 dark:divide-gray-700">

        <li v-for="obj in results" :key="obj.id + userState.onlineUsers" class="flex gap-4 items-center">
          <CheckBox :id="`user_${obj.id}`" :checked="selected.indexOf(obj.id) > -1" @update="selectionUpdated"
                    :value="obj.id"/>
          <label :for="`user_${obj.id}`" class="flex gap-2 items-center text-gray-900 dark:text-white">
