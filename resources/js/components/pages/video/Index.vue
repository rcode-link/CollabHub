...
const toggleCamera = async () => {
  liveKit.myDevicesState.isCameraEnabled =
    !liveKit.myDevicesState.isCameraEnabled;
  await liveKit.room.value.localParticipant.setCameraEnabled(
    liveKit.myDevicesState.isCameraEnabled
  );
};
const toggleMicrophone = async () => {
  liveKit.myDevicesState.isMicrophoneEnabled =
    !liveKit.myDevicesState.isMicrophoneEnabled;
  await liveKit.room.value.localParticipant.setMicrophoneEnabled(
    liveKit.myDevicesState.isMicrophoneEnabled
  );
};
//
...
const changeVideo = () => liveKit.switchDevice("videoinput", obj.deviceId);
const changeAudio = () => liveKit.switchDevice("audioinput", obj.deviceId);
</script>

<template>
  <div
    v-if="!model.token"
    class="z-[60] flex h-screen w-screen justify-center items-center"
  >
  ...
  >
    ...
    >
      ...
      <fwb-button
        :color="
          liveKit.myDevicesState.isCameraEnabled === false ? 'red' : 'default'
        "
        pill
        square
        @click="toggleCamera"
      >
        <CameraIcon
          v-if="liveKit.myDevicesState.isCameraEnabled !== false"
          class="w-6 h-6"
        />
        <CameraSlashIcon
          v-if="liveKit.myDevicesState.isCameraEnabled === false"
      ...
      <fwb-button
        :color="
          liveKit.myDevicesState.isMicrophoneEnabled === false
            ? 'red'
            : 'default'
        "
        pill
      ...
      >
        <MicrophoneIcon
          v-if="liveKit.myDevicesState.isMicrophoneEnabled !== false"
          class="w-6 h-6"
        />
        <MicrophoneSlashIcon
          v-if="liveKit.myDevicesState.isMicrophoneEnabled === false"
      ...
      <fwb-button
        :color="model.shareScreen === true ? 'red' : 'default'"
        pill
        square
        @click="
          () => {
          ...
          <div>
            <fwb-list-group>
              ...
              <fwb-list-group-item
                ...
                @click="changeVideo"
                v-for="(obj, i) in liveKit.videoDevices.value"
                :key="i"
              >
              ...
              <fwb-list-group-item
                class="cursor-pointer"
                @click="changeAudio"
                v-for="(obj, index) in liveKit.audioDevices.value"
                :key="index"
              >
                <span v-if="obj.active">&#10003;</span> {{ obj.label }}
              </fwb-list-group-item>
    ...
    >
      <TrackHandler
        ...
        :tracks="liveKit.tracks.value[item]"
      />
      <TrackHandler
        :enableAudio="false"
        user-image="https://imgs.search.brave.com/GrTMprW4fg05XTsfzacsNofnbaMJuXlbLIXZqUAn9vg/rs:fit:860:0:0/g:ce/aHR0cHM6Ly90NC5m/dGNkbi5uZXQvanBn/LzAwLzY0LzY3LzI3/LzM2MF9GXzY0Njcy/NzM2X1U1a3BkR3M5/a2VVbGw4Q1JRM3Az/WWFFdjJNNnFrVlk1/LmpwZw"
        :tracks="liveKit.localTracks.value"
      />
      ...
      <TrackHandler
        ...
        user-image="https://imgs.search.brave.com/GrTMprW4fg05XTsfzacsNofnbaMJuXlbLIXZqUAn9vg/rs:fit:860:0:0/g:ce/aHR0cHM6Ly90NC5m/dGNkbi5uZXQvanBn/LzAwLzY0LzY3LzI3/LzM2MF9GXzY0Njcy/NzM2X1U1a3BkR3M5/a2VVbGw4Q1JRM3Az/WWFFdjJNNnFrVlk1/LmpwZw"
        :tracks="
          liveKit.videoShareTrack.value.filter(
