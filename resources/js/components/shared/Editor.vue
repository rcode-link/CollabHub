...
const shouldISubmit = () => {
  if (
    props.submitOnEnter &&
    pressedKeys.value.indexOf('Enter') > -1 &&
    pressedKeys.value.indexOf('Shift') === -1
  ) {
    return true
  }

  if (!props.submitOnEnter && pressedKeys.value.join('-') === 'Control-Enter') {
    return true
  }
  return false
}
...
const editor = useEditor({
  ...
  editorProps: {
    handleDOMEvents: {
      keydown: (view, event) => {
        if (['Control', 'Enter', 'Shift'].indexOf(event.key) > -1) {
