<template>
  <router-link :to="goToRoute()"
    ...
    <div class="col-span-5 text-gray-900 dark:text-white">
      ...
      <div v-if="obj.message !== null" :key="JSON.stringify(obj.message)" class="text-xs grid grid-cols-6 gap-1">
        <div class="col-span-5 h-4 flex gap-4 overflow-hidden">
          {{ obj.message.user }}: {{ obj.message.text }}
        </div>
        <div class="col-span-1">
          <div v-if="daysAgo === 0">
            {{ DateTime.fromISO(obj.message.created_at).toLocaleString(DateTime.TIME_SIMPLE) }}
          </div>
          <div v-if="daysAgo === 1">
            {{ daysAgo }} day ago
          </div>
...
const daysAgo = computed(() => {
  return DateTime.now().startOf('day').diff(DateTime.fromISO(props.obj.message.created_at).startOf('day'), ['hours', 'days']).days
})
</script>
<style scoped>
.custom-list-item {
