...
const deleteRecord = ({ id }) => {
    axios.delete(`/api/v1/company/invite/${id}`).then(() => data());
};
</script>
...
<template>
    <fwb-table hoverable>
        <fwb-table-head>
            ...
            <fwb-table-head-cell
                >Available slots / Used slots</fwb-table-head-cell
            >
            ...
            <fwb-table-row v-for="(obj, index) in response.data" :key="obj.id">
                ...
                <fwb-table-cell>
                    <InteractiveToast>
                        ...
                        <template #content>
                            You are about to delete invitation link with order
                            number "{{ index + 1 }}"?
                        </template>
                        <template #actions>
                            <fwb-button
                                size="xs"
                                @click="() => deleteRecord(obj)"
                            >
                                Yes! Delete it.
                            </fwb-button>
                        </template>
