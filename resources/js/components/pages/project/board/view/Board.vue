...
onMounted(() => {
  Echo
      ...
      .listen('TaskUpdatedEvent', ({task, oldStatus}) => {
...
function manageBreadcrumb() {
  ...
  if (_.findIndex(breadcrumbStore.links, {'title': boardState.activeSprint.title}) === -1 && boardState.activeBoard.type === 'scrum') {
        breadcrumbStore.addEntry([
            {
                link: {
...
watch(() => boardState.activeBoard.id, (value, oldValue, onCleanup) => {
  loadStatuses();
  manageBreadcrumb();
}, {
  immediate: true
...
<template>
  <div v-if="boardState.getBoards.length > 0"
       :key="route.params.sprint"
       class="flex w-full gap-4 overflow-auto pb-4">
      <StatusRow v-for="status in boardState.taskStatuses" :key="status.id" :status="status"/>
    </div>
</template>
