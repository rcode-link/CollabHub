...
const loadUsers = () => {
    ...
    timer.value = setTimeout(() => {
      ...
      axios.get(`/api/v1/company/users/${userState.company.id}`, {
            params: {
...
<template>
    <div class="relative">
        <div
            ...
            'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-[0.4rem] dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500': !errors.errors.hasOwnProperty(name),
            'bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 dark:bg-gray-700 focus:border-red-500 block w-full p-[0.4rem] dark:text-red-500 dark:placeholder-red-500 dark:border-red-500': errors.errors.hasOwnProperty(name)
        }"
        ...
        <div v-show="showItems && !disabled" class="absolute w-full z-50 bg-white dark:bg-gray-700 shadow">
            ...
            <div v-for="obj in users.data" @click="() => handleItemClick(obj)"
