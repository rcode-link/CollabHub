<template>
    <Teleport :to="`#message-${message.id}`">
        <div
            class="flex gap-2 absolute top-0  right-0 p-2  bg-slate-600 text-white hover:text-slate-200 z-50 rounded-sm">
            ...
            <tippy content="Delete Message" placement="top">
                <TrashIcon class="w-4 h-4 cursor-pointer" @click="() => deleteMessageToast.showToastFn()" />
            </tippy>
        </div>
    </Teleport>
