...
watch(() => calendar.form.start_time, () => {
  if (calendar.form.end_time) {
    return;
  }
  calendar.form.end_time = DateTime.fromISO(calendar.form.start_time).set({
  ...
  <Modal :hide-modal="calendar.isModalVisible" @closed="calendar.closeModal">
    ...
    <template #body>
      <form @submit.prevent="calendar.save" class="flex flex-col gap-4">
        <fwb-select label="Type *" v-model="calendar.form.type" :options="[
          {
            value: 'event',
            name: 'Event'
          },
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="md:col-span-2">
            ...
            <DatePicker v-model="calendar.form.start_time" :name="'event_date'" />
            <Errors name="start_time" />
          </div>
          <div>
            <Label forInput="start_time">Start time *</Label>
            <TimePicker v-model="calendar.form.start_time" :name="'start_time'" />
            <Errors name="start_time" />
          </div>
          <div>
        ...
        <fwb-checkbox v-if="calendar.form.type !== 'vacation'" label="Advanced form"
          v-model="calendar.showAdvancedSettings" />
        <fwb-checkbox v-if="calendar.form.type !== 'vacation'" label="Has video conference"
          v-model="calendar.form.has_video" />
        <template v-if="calendar.showAdvancedSettings && calendar.form.type !== 'vacation'">
          <div>
          ...
          <fwb-select v-model="calendar.form.freq"
          ...
          <template v-if="calendar.form.freq === 'WEEKLY'">
            <div>
              ...
              <fwb-checkbox v-for="day in calendar.byDay" :label="day.label"
          ...
          <fwb-table hoverable>
            ...
            <fwb-table-body>
              <fwb-table-row v-for="(obj, index) in  calendar.form.users" :key="obj.id">
                ...
                <fwb-table-cell>
                  <fwb-button color="red" @click.prevent="() => calendar.form.users.splice(index, 1)">
    ...
    <template #footer>
      <div class="flex justify-between">
        <div>
          ...
          <FwbButton color="yellow" class="ml-4" @click="openCall"
            v-if="calendar.form.has_video && calendar.videoCall.slug">Start call
          </FwbButton>
        </div>
        <interactive-toast>
          <template #trigger>
          ...
          <template #actions>
            <FwbButton color="red" @click="calendar.deleteEvent" size="xs">Yes, Delete it</FwbButton>
          </template>
        </interactive-toast>
      </div>
    </template>
